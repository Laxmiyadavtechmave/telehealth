...
(function () {
  ...
  feather.replace(); // Page Content Height Resize
  ...
  if ($('.select-img').length > 0) {
    var formatState = function formatState(state) {
      ...
      var $state = $('<span><img src="' + $(state.element).attr('data-image') + '" class="img-flag" / " width="16px"> ' + state.text + '</span>');
      return $state;
    };
  ...
  var Colsidemenu = function Colsidemenu() {
  ...
  colinit(); // Table Responsive

  setTimeout(function () {
    $(document).ready(function () {
      $('.table').parent().addClass('table-responsive');
    });
  }, 1000); // Date Range Picker

  if ($('.bookingrange').length > 0) {
    var booking_range = function booking_range(start, end) {
  ...
  if ($('.daterange').length > 0) {
    $('.daterange').daterangepicker({
      ...
      // Prevents immediate update of input field
      ranges: {
        'Today': [moment(), moment()],
        'Yesterday': [moment().subtract(1, 'days'), moment().subtract(1, 'days')],
        'Last 7 Days': [moment().subtract(6, 'days'), moment()],
  ...
  $(document).on('click', '#toggle_btn', function () {
    if ($('body').hasClass('mini-sidebar')) {
      // Commented out the line that removes mini-sidebar
      ...
      setTimeout(function () {
        // $("body").removeClass("mini-sidebar"); // removed this too
  ...
  if ($('.custom-input').length > 0) {
    var inputRange = document.querySelector('.custom-input');
    inputRange.addEventListener('input', function () {
      var progress = (inputRange.value - inputRange.min) / (inputRange.max - inputRange.min) * 100;
      inputRange.style.background = "linear-gradient(to top, var(--md-sys-color-on-surface-variant) 0%, var(--md-sys-color-on-surface-variant) ".concat(progress, "%, var(--md-sys-color-surface-variant) ").concat(progress, "%, var(--md-sys-color-surface-variant) 100%)");
    });
  } // Datetimepicker time
  ...
  if ($('.btnFullscreen').length > 0) {
    ...
    var _toggleFullscreen = function _toggleFullscreen() {
      if (!document.fullscreenElement) {
        document.documentElement.requestFullscreen();
      } else {
        if (document.exitFullscreen) {
    ...
    var btnFullscreenElements = document.getElementsByClassName('btnFullscreen'); // Add an event listener to each element

    Array.from(btnFullscreenElements).forEach(function (element) {
